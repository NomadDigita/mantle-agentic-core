...
function FloatingGlassCard({ children, className, delay = 0, isAuraActive = true }: { children: React.ReactNode, className: string, delay?: number, isAuraActive?: boolean }) {
  ...
  const borderGradients = {
    IDLE: "bg-[conic-gradient(from_0deg,transparent_0%,#10b981_30%,transparent_50%,#3b82f6_80%,transparent_100%)]",
    LISTENING: "bg-[conic-gradient(from_0deg,transparent_0%,#10b981_20%,transparent_50%,#047857_80%,transparent_100%)]",
    ANALYZING: "bg-[conic-gradient(from_0deg,transparent_0%,#f59e0b_30%,transparent_50%,#ef4444_80%,transparent_100%)]",
    MINTING: "bg-[conic-gradient(from_0deg,transparent_0%,#a855f7_30%,transparent_50%,#6366f1_80%,transparent_100%)]",
    OVERCLOCK: "bg-[conic-gradient(from_0deg,transparent_0%,#ef4444_30%,transparent_50%,#7f1d1d_80%,transparent_100%)]"
  };
...
function IntroSequence({ onComplete, isAuraActive = true }: { onComplete: () => void, isAuraActive?: boolean }) {
  return (
    <motion.div 
    ...
    >
      <FloatingGlassCard isAuraActive={isAuraActive} className="max-w-2xl w-full text-center">
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
          <div className="w-24 h-24 mx-auto mb-6 rounded-3xl bg-gradient-to-br from-emerald-400 to-blue-500 blur-[2px] shadow-[0_0_40px_rgba(16,185,129,0.5)] flex items-center justify-center border-2 border-white/20">
            <div className="w-12 h-12 bg-black rounded-xl animate-pulse" />
          </div>
          <h1 className="text-4xl font-black text-white uppercase tracking-widest mb-2">Mantle <span className="text-emerald-400">Agentic</span> Core</h1>
          <p className="text-white/50 font-mono text-sm mb-10">Autonomous Web3 Operations Ecosystem</p>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10 text-left">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="bg-black/40 p-5 rounded-2xl border border-white/5">
            <h3 className="text-emerald-400 font-bold text-xs uppercase mb-2 tracking-widest">01. Live Terminal</h3>
            <p className="text-white/40 text-[10px] font-mono leading-relaxed">Real-time market analysis and agentic execution.</p>
          </motion.div>
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="bg-black/40 p-5 rounded-2xl border border-white/5">
            <h3 className="text-amber-400 font-bold text-xs uppercase mb-2 tracking-widest">02. Neural Forge</h3>
            <p className="text-white/40 text-[10px] font-mono leading-relaxed">Autonomous smart contract compilation and auditing.</p>
          </motion.div>
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.6 }} className="bg-black/40 p-5 rounded-2xl border border-white/5">
            <h3 className="text-blue-400 font-bold text-xs uppercase mb-2 tracking-widest">03. Citadel Vault</h3>
            <p className="text-white/40 text-[10px] font-mono leading-relaxed">ERC-8004 Agent Identity minting and risk management.</p>
          </motion.div>
        </div>
        ...
        <motion.button 
          ...
          onClick={onComplete}
          className="bg-white text-black font-black uppercase tracking-[0.2em] text-xs px-12 py-4 rounded-full hover:bg-emerald-400 hover:shadow-[0_0_30px_rgba(16,185,129,0.5)] transition-all transform hover:scale-105 active:scale-95"
        >
          Initialize System
        </motion.button>
      </FloatingGlassCard>
...
function ReasoningLogsHUD({ steps, latency }: { steps: string[], latency?: string }) {
  ...
  return (
    <div className="border border-white/10 rounded-xl overflow-hidden bg-black/40 mb-2">
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex justify-between items-center px-4 py-2 text-[10px] font-mono text-purple-400 hover:bg-white/5 transition-all uppercase tracking-widest border-b border-white/5"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="p-4 space-y-2 font-mono text-[9px] text-white/50 leading-relaxed border-t border-white/5">
              {steps.map((step, idx) => (
                <div key={idx} className="flex gap-2 items-start">
                  <span className="text-purple-500/80">[{idx + 1}]</span>
...
export default function Home() {
  ...
  const { isOverclocked, toggleOverclock, safeColors, setSystemState, systemState } = useTheme();
  const { primary, secondary, border, glow, dotBg } = safeColors;
  ...
  // AURA MATRIX CHIP SELECTOR STATE
  ...
  useEffect(() => {
    const lines = ["> SCANNING MANTLE MEMPOOL...", "> ANALYZING L2 LIQUIDITY...", "> ERC-8004 PROTOCOLS ACTIVE...", "> WAITING FOR EXECUTIVE COMMAND..."];
    let i = 0;
    const interval = setInterval(() => { i = (i + 1) % lines.length; setHeaderText(lines[i]); }, 4000);
    return () => clearInterval(interval);
  }, []);

  const marketCoins = [
    { symbol: 'BTC', pair: 'BTCUSDT', name: 'BITCOIN', color: 'text-emerald-400', glow: 'shadow-[0_0_15px_rgba(16,185,129,0.5)]', border: 'border-emerald-500/50', bg: 'bg-emerald-500/20' },
    { symbol: 'ETH', pair: 'ETHUSDT', name: 'ETHEREUM', color: 'text-blue-400', glow: 'shadow-[0_0_15px_rgba(59,130,246,0.5)]', border: 'border-blue-500/50', bg: 'bg-blue-500/20' },
    { symbol: 'SOL', pair: 'SOLUSDT', name: 'SOLANA', color: 'text-amber-500', glow: 'shadow-[0_0_15px_rgba(245,158,11,0.5)]', border: 'border-amber-500/50', bg: 'bg-amber-500/20' }
  ];
  ...
  useEffect(() => {
    const simulateLivePrices = () => {
      ...
      Object.keys(basePrices.current).forEach(pair => {
        ...
        const volatility = base * (Math.random() * 0.003 - 0.0015);
  ...
  return (
    <main className="min-h-screen relative p-4 md:p-8 lg:p-12 z-0 overflow-x-hidden bg-transparent font-sans">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none -z-20 opacity-40">
        <div className="absolute top-[10%] left-[20%] w-[45vw] h-[45vw] bg-emerald-500/10 rounded-full blur-[140px] animate-[pulse_8s_ease-in-out_infinite]" />
        <div className="absolute bottom-[10%] right-[10%] w-[55vw] h-[55vw] bg-blue-500/10 rounded-full blur-[160px] animate-[pulse_10s_ease-in-out_infinite_delay-2s]" />
        
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_20%,#000_100%)]" />
        
        <div className="absolute top-[20%] left-[30%] w-2 h-2 rounded-full bg-emerald-400 animate-ping [animation-duration:5s]" />
        <div className="absolute top-[65%] left-[15%] w-1.5 h-1.5 rounded-full bg-blue-400 animate-ping [animation-duration:7s]" />
        <div className="absolute top-[45%] left-[80%] w-2 h-2 rounded-full bg-purple-400 animate-ping [animation-duration:6s]" />
        <div className="absolute top-[80%] left-[60%] w-1 h-1 rounded-full bg-emerald-500 animate-ping [animation-duration:8s]" />
      ...
      >
        <div className={`flex justify-between items-center mb-8 bg-white/5 backdrop-blur-xl border ${border} p-6 rounded-3xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] transition-colors duration-500`}>
          ...
          <div className="flex gap-4">
            ...
            >
              {isAuraActive ? 'AURA MATRIX: ON' : 'AURA MATRIX: OFF'}
            </button>
             <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        {msg.actionPayload && (
                          <div className={`mt-4 p-5 rounded-xl border backdrop-blur-md ${msg.actionPayload.status === 'SUCCESS' ? 'bg-emerald-500/10 border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.2)]' : 'bg-black/40 border-white/10'}`}>
                            ...
                            <div className="mb-5 bg-white/5 p-4 rounded-lg border border-white/5 flex gap-4 items-start">
                                ...
                                <div>
                                    <span className="text-[9px] uppercase tracking-widest text-white/40 block mb-1">STRATEGY ANALYSIS</span>
                                    <p className="text-xs text-white/70 font-mono leading-relaxed">{msg.actionPayload.analysis}</p>
                                </div>
                            </div>
                            ...
                            {msg.actionPayload.status !== 'SUCCESS' ? (
                              <div className="flex gap-3">
                                <button 
                                  ...
                                  className="flex-1 bg-purple-600 hover:bg-purple-500 disabled:bg-purple-600/50 text-white font-bold text-xs py-3 rounded-xl uppercase tracking-widest shadow-[0_0_20px_rgba(147,51,234,0.3)] transition-all active:scale-95"
                                >
                                  {isTradeConfirming ? "WRITING TO LEDGER..." : "CONFIRM & LOCK COLLATERAL"}
                                </button>
                                <button className="px-6 bg-transparent border border-white/10 text-white/50 hover:bg-white/5 font-bold text-xs rounded-xl uppercase tracking-widest transition-all">Cancel</button>
                              </div>
                            ) : (
                              <div className="w-full bg-emerald-500/20 border border-emerald-500/50 text-emerald-400 text-center font-bold text-xs py-3 rounded-xl uppercase tracking-widest flex flex-col gap-1">
                                ...
                                <a 
                                  ...
                                  className="text-[9px] text-purple-400 underline hover:text-purple-300 font-mono"
                                >
          ...
          <div className="space-y-8">
            ...
            <FloatingGlassCard isAuraActive={isAuraActive} delay={0.2} className={`bg-white/5 backdrop-blur-3xl p-8 border ${border} rounded-3xl shadow-[0_30px_60px_rgba(0,0,0,0.5)] transition-colors duration-500`}>
              ...
              {isCheckingAgent ? (
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center border-b border-white/5 pb-2">
                    <span className="text-[10px] text-white/50 uppercase font-mono">RISK SETTING:</span>
                    <span className="text-xs font-bold text-purple-400 drop-shadow-[0_0_10px_rgba(168,85,247,0.4)]">{agentProfile.riskStrategy}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-white/5 pb-2">
                    <span className="text-[10px] text-white/50 uppercase font-mono">MAX ALLOWABLE DRAWDOWN:</span>
                    <span className="text-xs font-bold text-red-400">{agentProfile.maxDrawdown}%</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-white/5 pb-2">
                    <span className="text-[10px] text-white/50 uppercase font-mono">MAX LEVERAGE LIMIT:</span>
                ...
                <div className="text-center py-4">
                  <p className="text-xs text-white/40 mb-3">No active ERC-8004 identity found mapped to your address.</p>
                  <Link href="/citadel">
                    <button className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-bold uppercase tracking-widest text-emerald-400 hover:bg-white/10 transition-colors">
                      Awaken Identity
                    </button>
                  </Link>
                </div>
            ...
            <AnimatePresence>
              {yieldWeaverMode !== "IDLE" && (
                ...
                >
                  <FloatingGlassCard isAuraActive={isAuraActive} delay={0.3} className="bg-black/40 border border-purple-500/30 rounded-3xl p-6 shadow-[0_0_30px_rgba(168,85,247,0.2)]">
                    <div className="flex justify-between items-center border-b border-white/10 pb-3 mb-4">
                      ...
                      <span className="text-[9px] font-mono text-white/40">Active: Mantle RWA</span>
                    </div>

                    <div className="space-y-3 mb-6 font-mono text-xs">
                      <div className="flex justify-between">
                        <span className="text-white/40">Ondo USDY APY:</span>
                        <span className="text-white font-bold">5.1% APY</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-white/40">Mantle mETH APY:</span>
                        <span className="text-emerald-400 font-bold">7.2% APY</span>
                      </div>
                      <div className="flex justify-between border-t border-white/5 pt-2">
                        <span className="text-white/40">WEAVER ALLOCATION:</span>
                    ...
                    {yieldWeaverMode !== "mETH_PREMIUM" && (
                      <button
                        ...
                        disabled={isWeavingTx}
                        className="w-full py-3 rounded-xl bg-purple-600 hover:bg-purple-500 font-black text-[10px] text-white uppercase tracking-widest shadow-[0_0_20px_rgba(147,51,234,0.4)] transition-all active:scale-95"
                      >
            ...
            <AnimatePresence>
               {isConnected && address && mntGasBalance < 1.0 && (
                 <motion.div
                   ...
                   transition={{ duration: 0.4 }}
                 >
                   <FloatingGlassCard isAuraActive={isAuraActive} delay={0.2} className="bg-black/40 border border-amber-500/30 rounded-3xl p-6 shadow-[0_0_30px_rgba(245,158,11,0.25)] animate-pulse">
                     <div className="flex justify-between items-center border-b border-white/10 pb-3 mb-4">
            ...
            <FloatingGlassCard isAuraActive={isAuraActive} delay={0.1} className={`bg-white/5 backdrop-blur-3xl p-8 border ${border} rounded-3xl shadow-[0_30px_60px_rgba(0,0,0,0.5)] transition-colors duration-500`}>
            ...
            <FloatingGlassCard isAuraActive={isAuraActive} delay={0.2} className="bg-white/5 border border-white/10 rounded-3xl p-6 h-[220px]">
               <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-3 border-b border-white/10 pb-2 flex items-center gap-2 flex-shrink-0">
                 <div className="w-1.5 h-1.5 rounded-full bg-purple-500 animate-ping" />
                 Mantle Agent Matrix Relay
               </h4>
               <div className="font-mono text-[9px] space-y-3 leading-relaxed text-white/60 overflow-y-auto scrollbar-hide flex-1 max-h-[140px]">
                 <AnimatePresence>
                   {relayLogs.map((log) => (
                     <motion.div 
                       key={log.text}
            ...
            <FloatingGlassCard isAuraActive={isAuraActive} delay={0.4} className={`bg-white/5 backdrop-blur-3xl p-8 border ${border} rounded-3xl shadow-[0_30px_60px_rgba(0,0,0,0.5)] transition-colors duration-500`}>
              <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/40 mb-8 border-b border-white/10 pb-4">
                Node Topology
              </h4>
              <div className="space-y-4">
                ...
                <div className="bg-black/30 border border-white/5 rounded-2xl p-5 flex justify-between items-center backdrop-blur-md shadow-[inset_0_0_15px_rgba(255,255,255,0.02)]">
                  <div>
                    ...
                    <p className="text-[10px] font-mono text-white/40">{isConnected ? `${address?.slice(0, 6)}...${address?.slice(-4)}` : "Standby"}</p>
                  </div>
            ...
            <FloatingGlassCard isAuraActive={isAuraActive} delay={0.6} className={`bg-transparent rounded-3xl shadow-[0_30px_60px_rgba(0,0,0,0.5)]`}>
              <div className="relative overflow-hidden rounded-3xl h-full w-full">
                <div className="relative h-full w-full">
                  <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
                     <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/40">Market Sentinel</span>
                  ...
                  <div className="flex gap-3">
                     <button className={`flex-1 ${currentMarket.bg} ${currentMarket.color} border ${currentMarket.border} rounded-xl py-3 text-xs font-black uppercase tracking-widest hover:bg-white/10 transition-all transform hover:-translate-y-1 active:scale-95`}>Long</button>
                     <button className="flex-1 bg-red-500/20 text-red-400 border border-red-500/50 rounded-xl py-3 text-xs font-black uppercase tracking-widest hover:bg-red-500/40 hover:shadow-[0_0_20px_rgba(239,68,68,0.3)] transition-all transform hover:-translate-y-1 active:scale-95">Short</button>
                  </div>
                </div>
              </div>
            </FloatingGlassCard>

            <FloatingGlassCard isAuraActive={isAuraActive} delay={0.4} className="bg-transparent h-[80px]">
              <button 
                onClick={() => open()} 
                className="w-full h-full rounded-2xl bg-white text-black hover:bg-emerald-400 transition-colors font-black text-xs uppercase tracking-[0.2em] shadow-[0_20px_40px_rgba(255,255,255,0.1)] hover:shadow-[0_20px_40px_rgba(16,185,129,0.3)] active:scale-95"
              >
